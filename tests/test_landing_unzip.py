...
# The Files API double lives in tests/conftest.py (fake_workspace_client) so the
# giants tests exercise the same recording behaviour.


def test_upload_to_volume_returns_target_when_remote_size_matches(tmp_path, fake_workspace_client):
    src = tmp_path / "Estabelecimentos1.zip"
    src.write_bytes(b"z" * 4096)
    w = fake_workspace_client(4096)
...
def test_upload_to_volume_raises_when_the_remote_object_is_short(tmp_path, fake_workspace_client):
...
def test_upload_to_volume_raises_when_content_length_is_missing(tmp_path, fake_workspace_client):
    """No content-length means the write could not be confirmed -- that is a
    verification failure, not a pass."""
    src = tmp_path / "Estabelecimentos1.zip"
    src.write_bytes(b"z" * 4096)
    w = fake_workspace_client(None)

    with pytest.raises(UploadIntegrityError, match="Estabelecimentos1.zip"):
        upload_to_volume(w, src, "/Volumes/workspace/default/landing/zips")


def test_upload_to_volume_deletes_the_short_object_before_raising(tmp_path, fake_workspace_client):
...
def test_upload_to_volume_deletes_and_reraises_when_the_put_itself_fails(
    ...
    w = fake_workspace_client(4096, upload_error=boom)
...
def test_upload_to_volume_deletes_and_reraises_when_the_verification_call_fails(
    tmp_path, fake_workspace_client
):
    ...
    w = fake_workspace_client(4096, metadata_error=boom)
...
def test_upload_to_volume_does_not_delete_when_the_local_file_cannot_be_opened(
    tmp_path, fake_workspace_client
):
...
def test_upload_to_volume_surfaces_the_integrity_error_when_cleanup_fails(
    tmp_path, capsys, fake_workspace_client
):
    ...
    src = tmp_path / "Estabelecimentos1.zip"
    src.write_bytes(b"z" * 4096)
    w = fake_workspace_client(3072, delete_error=OSError("503 Service Unavailable"))
...
def test_upload_to_volume_reports_the_expected_404_as_nothing_to_delete(
    tmp_path, capsys, fake_workspace_client
):
    ...
    src = tmp_path / "Estabelecimentos1.zip"
    src.write_bytes(b"z" * 4096)
    w = fake_workspace_client(3072, delete_error=NotFound("File not found"))
