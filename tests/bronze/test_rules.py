...
def test_estabelecimentos_rules_evaluate():
    spark = local_session("test-estab-rules")
    try:
...
def test_lookup_default_golden_unchanged():
    """evaluate() with NO rules arg must behave exactly as F1.2 shipped."""
    spark = local_session("test-lookup-golden")
    try:
