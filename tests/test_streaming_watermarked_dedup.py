...
"""The half of T4+T5 that needs no broker: the boundary ARITHMETIC, the refusals, and the
three counts the dedup is read off.
...
is left to confirm it rather than to produce it.
...
THE CORPUS IS BUILT ONCE AT MODULE LEVEL and costs a few seconds -- 10,000 events derived
twice, and `opl.generator.profiles` explains why. That is the price of asserting over the
real declared corpus rather than over a miniature that would need numbers of its own.
"""
...
def _arm(name: str, *, landed: int, input_rows: int = 10_150, batches=(0, 1)) -> LandedArm:
    return LandedArm(
        arm=name,
        delay_ms=1,
        batch_ids=batches,
...
def _identities_the_model_removes(records, *, per_trigger: int, delay_ms: int) -> set[str]:
    ...
    `wd._batch_frontiers` and not a second spelling of it, so what is simulated is the
...
def _positions_whose_margin_equals_a_delay(*, per_trigger: int, lag: int, delays) -> list[int]:
    ...
    `wd._batch_frontiers` is read at a lag this function supplies, so the claim can be
    ...
    with pytest.MonkeyPatch.context() as patched:
        ...
        frontiers = wd._batch_frontiers(_EVENTS, per_trigger)
    return [
        position
...
def test_the_margins_are_the_lags_a_batched_reader_actually_sees():
    ...
    assert margins.widest_punctual_ms == wd._UNREACHABLE == -1, (
...
def test_the_first_batches_are_behind_a_floor_and_not_behind_the_first_record():
    ...
    frontiers = wd._batch_frontiers(_EVENTS, _PER_TRIGGER)
...
def test_the_correction_replaced_the_margin_function_and_not_one_constant(monkeypatch):
    ...
    monkeypatch.setattr(wd, "LATE_EVENT_WATERMARK_LAG_BATCHES", 1)
...
def test_a_corpus_with_no_late_arrivals_is_refused_rather_than_measured():
    ...
    with pytest.raises(ValueError, match="A corpus with nothing late"):
...
def test_a_dropping_delay_that_could_take_a_punctual_row_is_refused():
    ...
    refuse = wd._refuse_a_dropping_delay_that_reaches_a_punctual_row
...
def test_a_keeping_delay_equal_to_the_widest_margin_is_refused():
    ...
    refuse = wd._refuse_a_keeping_delay_that_does_not_clear_the_widest_margin
