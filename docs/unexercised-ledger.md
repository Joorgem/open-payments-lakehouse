...
**The last three point at a TEST module, and that is where those claims are verifiable rather than a filing convenience.** Each is a branch written in the commit that registers the table it will serve, over a population no YAML declares yet; the branch is the claim, and the module holding it is the only place the claim can be read against code. Both source docstrings say a row here is owed and name F2 wave 2's T4 as its owner — this is that row, and the docstring is the claim rather than the record. **`ventry` is the third and came the same way**: T3 measured an uncovered residual while closing its own review findings, declared it in the file that found it, and did not repair it there — the durable repair is a refusal in `src/opl/vault/`, so the comment is the claim and this is the debt.
...
| `ventry:486` | `_transactional_axis` returns the axis it is handed without checking | **nothing refuses a substituted axis on a transactional satellite.** `_resolved_parent` never reads the axis and `_transactional_axis` never compares it against `source_table`, so an in-place update of the seam's argument dict between `parent_arguments` and the loader leaves `tests/test_vault_entry_points.py` byte-identical to its honest headline while `snapshot_axis_for` answers `_snapshot_at` on a source that declares `_snapshot_month`. The mirror direction is worse: a monthly axis on a non-monthly source VALIDATES, because `_snapshot_month` is stamped on every bronze row whatever its source declares, and the fold then reads a column nothing keys on. **No volume of data closes it** — the exerciser is a source edit, and the repair is a refusal in `src/opl/vault/` rather than a stronger test. Measured and declared by F2 wave 2's T3 in `7b4b925`, in the file that found it and not repaired there on purpose |
...
| `jobdem:185` | THE LINK BRANCH IS UNEXERCISED TODAY AND THIS DOCSTRING USED TO CLAIM OTHERWISE. | a job YAML task naming `sat_link_payment`, which T3 owns and which the workspace 403 blocks. Measured by F2 wave 2's T2 review: reverting `_satellite_key_columns` to `parent_hub` leaves the file green apart from the pre-existing red, so the branch kills zero tests today |
| `jobdem:203` | THE SATELLITE ARM READS `spec.parent` AND NOT `parent_hub(spec).name` SINCE F2 WAVE 2, | the same YAML task. The two spellings agree on every satellite a job loads today — a hub parent's name IS `spec.parent` — so nothing distinguishes them until a link-parented satellite gets one |
| `vwiring:534` | AND THIS BRANCH IS UNEXERCISED TODAY, WHICH IS SAID RATHER THAN IMPLIED. | the same YAML task. `spec.transactional` is False for every spec the grain sweep's loop ever sees, so deleting the two lines kills no test. Written in the commit that registers the table because the alternative is a red run whose remedy is an edit to the sweep as well as to the YAML |
| `vspecs:290` | So the delta is true in form and unexercised in substance | the first STATE satellite on a link. `link_payment` hashes `transaction_id` into its digest, so every payment is its own partition with exactly one `applied_date`, `lag` is always NULL, and no candidate has ever been dropped as unchanged — what makes a re-load append nothing is `loading._without_persisted`'s (key, `applied_date`) anti-join ALONE |
