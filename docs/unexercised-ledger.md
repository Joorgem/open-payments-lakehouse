...
| `paydom:159` | AND THE README'S LIMIT NARROWS RATHER THAN CLOSING. `fact_payment` still reads | re-pointing `fact_payment` at `link_payment` and `sat_link_payment`. **F2 wave 2's own declared gap, and explicitly OUT of its scope** (ADR 0022 Decision 7): it is a gold refactor with its own risk and its own decision. Until it happens the payment is in the vault and the FACT does not read it, so the README's *Honest limits* narrows rather than closing **AND THE RE-POINTING HIDES A PRIOR TASK NOBODY HAS WRITTEN, measured 2026-09-02 after the run.** `fact_payment` resolves both role keys AS OF each payment s own `event_time`, and `event_time` IS IN NEITHER VAULT TABLE: `link_payment` keys on `transaction_id` plus the two hub references, and `sat_link_payment` carries only `amount`, `currency`, `payment_method`. `payment_channel` is absent too. So the exerciser is NOT a gold refactor on its own -- the satellite must first carry the event instant, which is a VAULT change. Doing it in the stated order discovers that mid-task. Note also what would be LOST: `opl.gold.facts` reduces 40,150 bronze rows to <=1,024 distinct keys and broadcasts, explicitly to avoid a sort-merge shuffling 69.2M rows per role, and that analysis would have to be redone against link+satellite. The argument that survives is retention: `opl.bronze.retention` deletes, the vault is insert-only, and a gold table reading bronze is only rebuildable while bronze exists -- which is a reason to put `event_time` in the satellite, not a reason to re-point the fact today. |
