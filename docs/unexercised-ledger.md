...
| `jobdem:186` | THE LINK BRANCH IS UNEXERCISED TODAY AND THIS DOCSTRING USED TO CLAIM OTHERWISE. | a job YAML task naming `sat_link_payment`, which T3 owns and which the workspace 403 blocks. Measured by F2 wave 2's T2 review: reverting `_satellite_key_columns` to `parent_hub` leaves the file green apart from the pre-existing red, so the branch kills zero tests today |
