...
> above are part of why. The first reads exactly as written for the target that IS
> deployed. **Which target each of the other two is about is no longer argued here**, and
> the deletion is the correction: this note said "both are about a production target",
> replaced that with the third firing under **any** target declaring the securable, and
...
> and therefore SKIPS wherever there is none, which is every CI job that runs the
> suite — enforced by `tests/test_ci_deploy.py`, which refuses a job that both names
