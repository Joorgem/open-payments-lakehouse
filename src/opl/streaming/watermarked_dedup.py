...
--- THE THREE WAYS THIS EXPERIMENT PASSES WHILE MEASURING NOTHING -----------------------
...
Each is refused below, in shipped code, rather than remembered by whoever writes the test.
  ...
  2. A WATERMARK THAT NEVER ADVANCES DROPS NOTHING EITHER. The watermark is computed from
     ...
     REFUSED BY `_refuse_a_read_that_cannot_advance_a_watermark`.
...
THIS PARAGRAPH EXISTS BECAUSE THE PREDICTION IT CORRECTS WAS PUBLISHED WRONG FIRST. A model
paying for ONE batch of slack -- in effect "the watermark applied to batch N is computed
from batches 0..N-1", though the arithmetic it was actually written in is the last
paragraph's and not this module's -- predicted 100 dropped rows and the run dropped 97. It
...
`tests/test_streaming_watermarked_dedup.py` re-runs the superseded derivation and the
one-constant variant SEPARATELY, because a test that monkeypatched the constant and called
the result "the superseded model" would be describing a hybrid that never existed.
...
reader. IT IS NOT ON THE DELAY PATH: nothing in `src/` calls it, `boundary_for` reads
...
paragraph above is about the two being confused, and a reader who can see only the one the
...
DERIVED FROM THIS ONE, and the paragraph above is why: the first measure alone is a
...
# third section gives the three observations that fix it and the one they rule out.
...
def frontier_lags_ms(spec: StreamSpec, defects: DefectSpec) -> tuple[int, ...]:
    ...
    delay is derived from; the module docstring's third section records what happened the
    time these two were confused.

    THIS FUNCTION AND `_frontier_index` HAVE NO CALLER IN `src/`. They are the second
    instrument, not a step in the delay path, and the module docstring's fourth section
    says why they ship anyway."""
    late = late_positions(spec, defects)
    _refuse_a_corpus_with_nothing_to_drop(defects, len(late))
...
def _refuse_a_corpus_with_nothing_to_drop(defects: DefectSpec, injected: int) -> None:
    ...
    if defects.late_count > 0 and injected == defects.late_count:
        return
    raise ValueError(
        f"the defect spec declares {defects.late_count} late arrivals and {injected} "
        "emission indices were selected. A corpus with nothing late in it makes both "
        "watermark arms land identical rows, and their difference -- which is the whole "
        "measurement -- would be 0 and true."
    )
...
def _refuse_a_lag_that_is_not_a_lag(lags: tuple[int, ...]) -> None:
    ...
    `DefectSpec` accept, and `tests/test_streaming_watermarked_dedup.py` builds one.
...
def _refuse_a_read_that_cannot_advance_a_watermark(max_offsets_per_trigger: int | None) -> None:
    """TRAP 2, IN SHIPPED CODE. See the module docstring's second numbered paragraph."""
...
# `tests/test_streaming_watermarked_dedup.py` pins the VALUE.
_UNREACHABLE = -1
...
def _batch_frontiers(events: Sequence[int], per_trigger: int) -> list[int | None]:
    ...
    `tests/test_streaming_watermarked_dedup.py` pins it."""
    running, prefix = -1, []
    for event in events:
...
class LatenessBoundary:
    ...
    def dropped_rows(self) -> int:
        ...
        reads the declaration and neither the rate limit nor the margins, so it is tested
        only where it meets the run's difference between two landed counts.
...
def _refuse_a_keeping_delay_that_does_not_clear_the_widest_margin(
    ...
    `tests/test_streaming_watermarked_dedup.py` does."""
...
def boundary_for(
    ...
    _refuse_a_corpus_with_nothing_to_drop(defects, margins.late_identities)
